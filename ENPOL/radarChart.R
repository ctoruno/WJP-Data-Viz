ENPOL_radarChart <- function(
  ...
  axis_coords <- function(n_axis = nvertix){
    ...
    x2 <- (0.5 + central_distance)*cos(fi)
    y2 <- (0.5 + central_distance)*sin(fi)
  ...
  # Y-Axis labels
  axis_measure <- tibble(r         = seq(0, 0.5, 0.1),
                         parameter = rep(data %>% 
                                           filter(order_var == 1) %>% 
                                           distinct(axis_var) %>% 
                                           pull(axis_var),
                                         6)) %>%
    bind_cols(map_df(seq(0, 0.5, 0.1) + central_distance, 
                     text_coords) %>% 
  ...
  radar <- 
    ...
    ggplot(data = map_df(seq(0, 0.5, 0.10) + central_distance, circle_coords),
           aes(x = x, 
               y = y)) +
    
    # We draw the outter ring
    geom_polygon(data     = circle_coords(0.5 + central_distance),
                 linetype = "dotted",
                 color    = "#d1cfd1",
                 fill     = NA) +
    ...
    geom_polygon(data = map_df(seq(0, 0.5, 0.10) + central_distance, circle_coords) %>%
                   filter(r == 0.2),
                 fill      = NA,
                 linetype  = "solid",
                 color     = "#d1cfd1") +
