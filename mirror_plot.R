mirror_plot <- function(data2plot, label_title, comparison, label_value_left, label_value_right) { 
  ...
  p1 <- ggplot(data2plot)+ aes(x = Value, y = target, fill = {{comparison}})+
    ...
    geom_text(aes(label = {{label_value_left}}, x = 0.5), color = "black", size = 2) + 
    geom_text(aes(label = {{label_value_right}}, x = -0.5), color = "black", size = 2) + 
    ...
    theme(legend.position="none",
          ...
          axis.text.y=element_text(family = "Lato Regular", size = 6, hjust = 0.5),
          plot.background = element_rect(fill = "white", colour = "white"),
          plot.subtitle=element_text(family = "Lato Light Italic", size=6, hjust = 1,color="Black"),
          plot.title=element_text(family="Lato Black",size=10,hjust = 0.5,color="Black"),
  ...
  g_plot <- p1_g %>% 
    ...
    gtable_add_grob(textGrob("", gp = gpar(fontsize = 6, fontfamily="Lato Regular")), l = 7, t = 7) 
  # add the top label
